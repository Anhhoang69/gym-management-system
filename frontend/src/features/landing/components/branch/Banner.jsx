    ...
    <div className="relative h-[520px] w-full">
      ...
      <div className="absolute inset-x-0 bottom-0 px-4 pb-6 md:px-16 md:pb-10">
        <div className="mx-auto flex max-w-7xl flex-col gap-8 md:flex-row md:items-end md:justify-between">
          {/* LEFT */}
          <div className="mb-4 flex w-full flex-col space-y-3 text-center md:mb-0 md:w-2/3 md:space-y-4 md:text-left">
            {/* Breadcrumb */}
            {breadcrumb?.length > 0 && (
              <nav className="text-sm text-gray-300">
                {breadcrumb.map((b, i) => (
                  <span key={i}>
                    ...
                    {i < breadcrumb.length - 1 && <span className="mx-2 text-gray-400">›</span>}
                  </span>
                ))}
              </nav>
            )}

...
import { cilLockLocked, cilUser, cilEnvelopeOpen, cilPhone, cilCameraControl, cilMonitor, cilX, cilBadge } from "@coreui/icons"
import CIcon from "@coreui/icons-react"
import { getMyProfile, updateMyProfile, getLoginHistory, revokeSession, uploadImage } from "../services/profileService"
    ...
    try {
      ...
      if (data) {
        setUser(prev => ({
          ...
          trainerInfo: data.trainerInfo || prev.trainerInfo
        }))
      }
    } catch (e) {
  ...
  const handleUpdateProfile = async (e) => {
    ...
    try {
      await updateMyProfile({
        ...
        trainerProfile: user.trainerInfo // API requires trainerProfile in PUT
      })
    ...
    try {
      ...
      if (uploadedUrl) {
        await updateMyProfile({
          ...
          languagePreference: user.languagePreference,
          trainerProfile: user.trainerInfo
        })
        setUser(prev => ({
          ...prev,
          avatarUrl: uploadedUrl
  ...
  return (
    <div className="container-fluid p-0 d-flex flex-column" style={{ height: "calc(100vh - 120px)" }}>
      <div className="mb-3 flex-shrink-0">
        <h3 className="fw-bold mb-1">Hồ Sơ Huấn Luyện Viên</h3>
      </div>
      ...
      <CRow className="flex-grow-1 overflow-hidden m-0">
        ...
        <CCol md={8} className="h-100 d-flex flex-column p-0 ps-md-2 pe-1">
          <CNav variant="pills" className="mb-3 flex-shrink-0 flex-nowrap overflow-auto hide-scrollbar">
            <CNavItem>
              <CNavLink active={activeTab === 1} onClick={() => setActiveTab(1)} style={{ cursor: 'pointer', whiteSpace: 'nowrap' }}>
                <CIcon icon={cilUser} className="me-2" /> Thông tin chung
              </CNavLink>
            </CNavItem>
            <CNavItem>
              <CNavLink active={activeTab === 2} onClick={() => setActiveTab(2)} style={{ cursor: 'pointer', whiteSpace: 'nowrap' }}>
                <CIcon icon={cilBadge} className="me-2" /> Hồ sơ năng lực
              </CNavLink>
            </CNavItem>
            <CNavItem>
              <CNavLink active={activeTab === 3} onClick={() => setActiveTab(3)} style={{ cursor: 'pointer', whiteSpace: 'nowrap' }}>
                <CIcon icon={cilLockLocked} className="me-2" /> Bảo mật
              </CNavLink>
            </CNavItem>
            <CNavItem>
              <CNavLink active={activeTab === 4} onClick={() => setActiveTab(4)} style={{ cursor: 'pointer', whiteSpace: 'nowrap' }}>
            ...
            <CCardBody className="p-4 overflow-auto custom-scrollbar h-100">
              <CTabContent>
                ...
                <CTabPane visible={activeTab === 2}>
                  <h5 className="fw-bold mb-4">Hồ sơ năng lực HLV</h5>
                  <CForm onSubmit={handleUpdateProfile}>
                    <CRow className="mb-3">
                      <CCol md={6}>
                        <CFormInput
                          ...
                          type="number"
                          min="0"
                          value={user.trainerInfo?.experienceYears || 0}
                          onChange={(e) => handleTrainerInfoChange('experienceYears', parseInt(e.target.value))}
                        />
                      </CCol>
                      <CCol md={6}>
                        <CFormInput
                          label="Chuyên môn"
                          placeholder="VD: Bodybuilding, Yoga, v.v..."
                          value={user.trainerInfo?.specialization || ""}
                          onChange={(e) => handleTrainerInfoChange('specialization', e.target.value)}
                        />
                      </CCol>
                    </CRow>
                    <CRow className="mb-3">
                      <CCol md={12}>
                        <CFormInput
                          label="Chứng chỉ"
                          placeholder="VD: NASM, ACE, v.v..."
                          value={user.trainerInfo?.certificate || ""}
                          onChange={(e) => handleTrainerInfoChange('certificate', e.target.value)}
                        />
                      </CCol>
                    </CRow>
                    <CRow className="mb-4">
                      <CCol md={12}>
                        <CFormTextarea
                          ...
                          rows={4}
                          value={user.trainerInfo?.bioDescription || ""}
                          onChange={(e) => handleTrainerInfoChange('bioDescription', e.target.value)}
                        />
                      </CCol>
                    </CRow>
                    <div className="text-end">

  ...
  useEffect(() => {
    const fetchReports = async () => {
      ...
      try {
        ...
        const now = new Date()
        if (filter === "month") {
          params = { month: now.getMonth() + 1, year: now.getFullYear(), branchId: myBranchId }
        } else if (filter === "quarter") {
          const fromMonth = Math.floor(now.getMonth() / 3) * 3 + 1
          params = { 
            fromDate: new Date(now.getFullYear(), fromMonth - 1, 1).toISOString(), 
            toDate: new Date(now.getFullYear(), fromMonth + 2, 0).toISOString(),
            branchId: myBranchId
          }
        } else if (filter === "year") {
          params = { year: now.getFullYear(), branchId: myBranchId }
        }

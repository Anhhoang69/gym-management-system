...
function OwnerPayrollPage() {
  ...
  useEffect(() => {
    setPayrollPage(1)
  }, [selectedPeriod, selectedBranchId, filterPosition, filterStatus])
  ...
  }, [activeTab, selectedPeriod, selectedBranchId, filterPosition, filterStatus, commPage, payrollPage])
  ...
  const handleApprovePeriod = async () => {
    const branchName = selectedBranchId 
      ? branches.find(b => b.branchId === selectedBranchId)?.name || "Chi nhánh đã chọn"
      : "tất cả chi nhánh"
      
    if (!window.confirm(`Bạn có chắc chắn muốn PHÊ DUYỆT toàn bộ bảng lương kỳ ${selectedPeriod} của ${branchName} không? Hành động này sẽ khóa và chuyển trạng thái chi trả thành ĐÃ PHÊ DUYỆT.`)) return
    setActionLoading(true)
    try {
      const [yearStr, monthStr] = selectedPeriod.split("-")
      const month = parseInt(monthStr, 10)
      const year = parseInt(yearStr, 10)
      const res = await approvePayrollPeriod(month, year, selectedBranchId || undefined)
      if (res.success) {
  ...
  const handleExportCsv = async () => {
    ...
    try {
      ...
      const month = parseInt(monthStr, 10)
      const year = parseInt(yearStr, 10)
      const res = await exportPayrollCsv(month, year, selectedBranchId || undefined)
      if (!res.success) {
  ...
  return (
    <div className="d-flex flex-column gap-4" style={{ minHeight: "calc(100vh - 120px)" }}>
      ...
      <div className="flex-shrink-0 d-flex justify-content-between align-items-center">
        <div>
          <h3 className="fw-bold mb-1">Duyệt & Quản Lý Payroll (Owner View)</h3>
          <p className="text-muted mb-0">Xem bảng thù lao toàn hệ thống, phê duyệt thanh toán định kỳ cho tất cả chi nhánh.</p>
        </div>
      </div>
      ...
      <CCard className="border-0 shadow-sm rounded-4 flex-shrink-0">
        <CCardBody className="p-3">
          <CRow className="align-items-end g-3">
            ...
            <CCol md={3} xs={12}>
              <CFormLabel className="small fw-bold mb-1">Chọn Chi Nhánh (Branch)</CFormLabel>
              <CFormSelect
          ...
          <CCard className="border-0 shadow-sm rounded-4 flex-shrink-0">
            <CCardBody className="p-3 d-flex justify-content-between align-items-center flex-wrap gap-2">
              <div className="d-flex align-items-center gap-2">
                <span className="small text-muted fw-bold">Công thức tính:</span>
                <CFormSelect
                  style={{ width: "230px" }}
